...
[
  {
    "member_id": "string (required)",
    "shift_template_id": "string (required)",
...
**CSV 格式規範**：
```csv
member_id,member_name,date,shift_template_id,shift_template_name
...
必要欄位：
- `member_id`: 成員唯一識別碼
- `member_name`: 成員顯示名稱（參考用）
- `date`: 排班日期（YYYY-MM-DD 格式）
- `shift_template_id`: 班別模板識別碼
- `shift_template_name`: 班別模板名稱（參考用）
...
### v0.5.0 (2025-01-17) - 最新版本
- ✅ **排班管理系統重大升級**
  - 新增月份批量排班功能，支援一次性安排整月班表
  - 實作 CSV 匯入排班功能，支援批量資料匯入
  - 提供標準 CSV 範本下載和即時驗證
  - 視覺化月曆介面，支援多日期選擇和批量指派
  - 自動檢測排班衝突和重複指派
  - 支援批量 API 調用，提升大量資料處理效率
