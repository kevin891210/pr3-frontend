    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="users" className="space-y-4">
          {selectedBrand && (
            <BrandUserManagement brandId={selectedBrand.id} />
          )}
        </TabsContent>
      </Tabs>
...
// Brand User Management Component
const BrandUserManagement = ({ brandId }) => {
  ...
  const fetchBrandUsers = async () => {
    setLoading(true);
    try {
      const response = await apiClient.getBrandUsers(brandId);
      setUsers(response.data || response || []);
    } catch (error) {
      console.error('Failed to fetch brand users:', error);
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  const addUser = async (userId) => {
    try {
      await apiClient.addBrandUser(brandId, userId);
  ...
  const removeUser = async (userId) => {
    try {
      await apiClient.removeBrandUser(brandId, userId);
      await fetchBrandUsers();
    } catch (error) {
  ...
  const availableUsers = allUsers.filter(user => 
    !users.some(brandUser => brandUser.id === user.id)
  );
      ...
      <CardContent>
        {loading ? (
          ...
          <EmptyState 
            ...
            description="No users have access to this brand yet." 
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              <h3 className="text-lg font-semibold mb-4">Add User to Brand</h3>
